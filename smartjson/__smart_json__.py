...
class SmartJsonError(Exception):
    ...
    def __str__(self):
        if six.PY2:
            ...
            if self.original_exception:
                return "{self.message} (Caused by: {type(self.original_exception).__name__} - {self.original_exception})".format(self=self)
            return self.message
...
class _KObject(object):
    def __init__(self, d):
        if not isinstance(d, dict):
            raise SmartJsonDeserializationError("Cannot create _KObject from type '{}'. Expected a dictionary structure.".format(type(d).__name__))
        for a, b in six.iteritems(d):
            try:
                ...
                raise SmartJsonDeserializationError("Error processing attribute '{}' for _KObject".format(a), original_exception=e)
    ...
    def self_dump(self, obj):
        ...
        if class_name in self.self_dumpers:
            try:
                ...
                raise SmartJsonSerializationError("Custom dumper for '{}' failed".format(class_name), original_exception=e)
        raise SmartJsonUnsupportedTypeError("Object of type '{}' is not JSON serializable with a custom dumper and no default path available in self_dump.".format(class_name))
    ...
    def json_convert(self, obj):
        ...
        if obj_id in self.visited:
            raise SmartJsonCircularDependencyError(
                "Circular dependency detected in json_convert for object type '{}' (id: {})".format(type(obj).__name__, obj_id),
                object_id=obj_id
            )
...
class _BaseConversion(object):
    ...
    def _convert_value(self, value):
        ...
        if obj_id in self.visited:
            raise SmartJsonCircularDependencyError(
                "Circular dependency detected in _convert_value for object type '{}' (id: {})".format(type(value).__name__, obj_id),
                object_id=obj_id
            )
        self.visited.add(obj_id)
        try:
            if isinstance(value, (list, tuple)):
                ...
                return _DataTypeConversion(value, self.visited).convert().__dict__ # Use module-level _DataTypeConversion
            return value
        finally:
            if obj_id in self.visited:
                self.visited.remove(obj_id)
...
class _DataTypeConversion(_BaseConversion):
    ...
    def convert(self):
        ...
        if obj_id in self.visited:
            raise SmartJsonCircularDependencyError(
                "Circular dependency detected for object of type '{}' (id: {})".format(type(self.___cls).__name__, obj_id),
                object_id=obj_id
            )
        self.visited.add(obj_id)
        try:
            ...
            raise SmartJsonSerializationError("Error converting attributes for '{}'".format(type(self.___cls).__name__), original_exception=e)
        finally:
            if obj_id in self.visited:
                self.visited.remove(obj_id)
...
class _EnumConversion(object):
    ...
    def convert(self):
        if self._json_cvt.get_class_name(self.__myEnum) == "enum.EnumMeta":
            ...
            raise SmartJsonUnsupportedTypeError("Type '{}' is not a directly serializable enum. Expected 'enum.EnumMeta'.".format(type(self.__myEnum).__name__))
...
class SmartJson(object):
    ...
    def serialize(self, pretty=True, schema=None):
        ...
        try:
            ...
            raise SmartJsonSerializationError("Failed to serialize object of type '{}'".format(obj_type), original_exception=e)
    ...
    def serializeToJsonFile(self, directory="output", filename="smart.json", schema=None):
        ...
        try:
            ...
            obj_type = type(self.__classe).__name__
            raise SmartJsonSerializationError("Failed to serialize object of type '{}' to file '{}'".format(obj_type, filepath), original_exception=e)
    ...
    def toObjectFromFile(self, jsonFile, schema=None):
        try:
            ...
            raise SmartJsonDeserializationError("Invalid JSON format in file '{}': {}".format(jsonFile, e.message), original_exception=e)
        except SmartJsonError:
            raise
        except Exception as e:
            raise SmartJsonDeserializationError("Error deserializing from file '{}'".format(jsonFile), original_exception=e)

    @staticmethod
    def _validate_object(obj, schema, path=""):
        if not isinstance(schema, dict):
            raise SmartJsonSchemaValidationError("Invalid schema definition at '{}': schema must be a dictionary.".format(path))
        ...
        for field_name, field_props in six.iteritems(schema):
            ...
            if not isinstance(field_props, dict):
                raise SmartJsonSchemaValidationError("Invalid schema definition for field '{}': properties must be a dictionary.".format(current_path))
            ...
            if is_required and not attr_exists:
                ...
                raise SmartJsonSchemaValidationError("Missing required attribute/key: '{}' on object/dict of type '{}'.".format(current_path, obj_type_name))
            if attr_exists:
                ...
                if expected_py_type:
                    if not isinstance(value, expected_py_type):
                        raise SmartJsonSchemaValidationError(
                            "Invalid type for attribute/key '{}'. Expected {}, got {}.".format(current_path, expected_py_type.__name__, type(value).__name__))
                    if 'schema' in field_props:
                        ...
                        if not isinstance(value, list):
                             raise SmartJsonSchemaValidationError("Attribute/key '{}' expected to be a list, got {}.".format(current_path, type(value).__name__))
                        for idx, item in enumerate(value):
                            item_path = "{}[{}]".format(current_path, idx)
                            if item_expected_py_type:
                                if not isinstance(item, item_expected_py_type):
                                    raise SmartJsonSchemaValidationError(
                                        "Invalid type for item at '{}'. Expected {}, got {}.".format(item_path, item_expected_py_type.__name__, type(item).__name__))
                            if item_schema_def:
                                if isinstance(item, (dict)) or hasattr(item, '__dict__'):
                                    ...
                                    raise SmartJsonSchemaValidationError(
                                        "Schema for item at '{}' provided, but item type '{}' is not an object or dictionary.".format(item_path, type(item).__name__))
        ...
        try:
            ...
            if isinstance(_json, six.string_types):
                ...
                raise SmartJsonDeserializationError(
                    "Invalid input type for deserialization: Expected a JSON string, bytes, or a dictionary, got {}.".format(type(_json).__name__))
            if schema and dic is not None:
                SmartJson._validate_data(dic, schema)
            return _KObject(dic) # Use module-level _KObject
        except json.JSONDecodeError as e:
            raise SmartJsonDeserializationError("Invalid JSON format in input: {}".format(e.message), original_exception=e)
    ...
    def _validate_data(data, schema, path=""):
        if not isinstance(schema, dict):
            raise SmartJsonSchemaValidationError("Invalid schema definition at '{}': schema itself must be a dictionary.".format(path))
        if not isinstance(data, dict):
            raise SmartJsonSchemaValidationError("Invalid data type at '{}'. Expected a dictionary, got {}.".format(path or "root", type(data).__name__))
        for field_name, field_props in six.iteritems(schema):
            current_path = "{}.{}".format(path, field_name) if path else field_name
            if not isinstance(field_props, dict):
                raise SmartJsonSchemaValidationError("Invalid schema definition for field '{}': properties must be a dictionary.".format(current_path))
            ...
            if field_name in data:
                ...
                if field_type_name:
                    ...
                    if not expected_type:
                        raise SmartJsonSchemaValidationError("Unknown type '{}' specified in schema for field '{}'.".format(field_type_name, current_path))
                    if not isinstance(value, expected_type):
                        ...
                        raise SmartJsonSchemaValidationError(
                            "Invalid type for field '{}'. Expected '{}', got '{}'.".format(current_path, field_type_name, actual_type_name))
                        ...
                        for idx, item in enumerate(value):
                            ...
                            if item_type_name:
                                ...
                                if not expected_item_type:
                                    raise SmartJsonSchemaValidationError(
                                        "Unknown item_type '{}' in list schema for field '{}'.".format(item_type_name, current_path))
                                if not isinstance(item, expected_item_type):
                                    ...
                                    raise SmartJsonSchemaValidationError(
                                        "Invalid type for item at '{}'. Expected '{}', got '{}'.".format(item_path, item_type_name, actual_item_type_name))
                            if item_schema_def:
                                if not isinstance(item, dict):
                                    raise SmartJsonSchemaValidationError(
                                        "Invalid item type at '{}'. Expected a dictionary for schema validation, got {}.".format(item_path, type(item).__name__))
